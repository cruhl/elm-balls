...
bounce {width, height} p =
    let
        ...
        (x', vx') = applyBoundaries (p.x, p.vx) (0, width')
        (y', vy') = applyBoundaries (p.y, p.vy) (0, height')
    in
        { p
        | x = x'
        , y = y'
...
view : Model -> Html Msg
view model = div [] (List.map renderParticle model.particles)
...
renderParticle {x, y, r} =
    let
        ...
        left = toString (x - r) |> addUnits
        bottom = toString (y - r) |> addUnits
        size = toString (r * 2) |> addUnits
    in
        div
            [ style
                [ "position" => "fixed"
